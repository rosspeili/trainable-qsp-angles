...
\fancyhead[R]{\small\color{graytext}Learning QSP Phase Angles --- Empirical Study}
...
\small
\textbf{Abstract.}\quad
Quantum Signal Processing (QSP) builds a one-qubit circuit that evaluates a polynomial in a scalar signal~$x$. Classically, the circuit's phase angles are computed by analytic solvers; here we \emph{learn} them from random initialization with gradient descent on a flat, JAX-traceable circuit (PennyLane is reference-only). On a standard degree-5 Chebyshev approximation of $\sin(x)$, Adam training reaches train MSE $9.6\times10^{-5}$ for seed~0---better than mapped analytic baselines ($\approx4.7\times10^{-3}$) on the same circuit---and 30/30 random seeds converge below $10^{-3}$. We report degree scaling, phase-convention mapping between solvers and the training circuit, and a QSVT-template pitfall that breaks JAX autodiff. Scripts, audit logs, and reproducible numbers are released under Apache~2.0.
...
\begin{itemize}\setlength\itemsep{3pt}
  ...
  \item[\textbf{C2.}] We provide a reproducible benchmark pipeline (30-seed statistics, scaling, ablations) for a degree-5 Chebyshev $\sin$ target, reporting convergence from MSE $\approx1.36$ to $9.6\times10^{-5}$ in 500 Adam steps (reference seed).
  \item[\textbf{C3.}] We compare gradient training to analytic baselines on equal footing---including convention mapping for flat-circuit evaluation---and provide an updated decision guide grounded in measured trade-offs.
...
Gradient-learned QSP phases are most compelling when the training objective \emph{is} the flat differentiable circuit---for example, QSP blocks inside variational pipelines with implicit targets. Immediate extensions include curriculum training across degree, tighter convention alignment for analytic baselines, and optional cross-SDK validation. The combination of interpretable polynomial structure and automatic differentiation remains a practical template for quantum subroutine design.
