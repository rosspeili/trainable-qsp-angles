...
\date{%
  ...
  {\normalfont\color{graytext}%
   ...
   \quad|\quad
   Data: \href{https://doi.org/10.5281/zenodo.20645403}{10.5281/zenodo.20645403}}%
}
...
\small
\textbf{Abstract.}\quad
Quantum Signal Processing (QSP) is a foundational algorithmic primitive that encodes polynomial transformations of a scalar signal into a single-qubit quantum circuit via a structured sequence of phase-shifted oracle calls. The canonical approach computes phase angles analytically from a target polynomial---a well-studied but numerically sensitive problem. We present a reproducible \emph{empirical study} of \emph{learning} QSP phase angles from random initialization using gradient-based optimization on a JAX-traceable \emph{flat} circuit (PennyLane is used only as a reference frontend). We benchmark a degree-5 Chebyshev approximation of $\sin(x)$ with Adam over 500 steps, multi-seed replication ($n=30$), degree scaling, and analytic baselines (PennyLane \texttt{poly\_to\_angles} and Chao et al.\ via \texttt{pyqsp}). For the reference seed, training reaches train MSE $9.6\times10^{-5}$; mapped analytic baselines on the same flat circuit achieve $\approx4.7\times10^{-3}$. We document phase-convention mapping between solver output and the training circuit, identify a QSVT-template pitfall for JAX tracing, and release experiment scripts, audit logs, and figure data under Apache~2.0 (Zenodo: \href{https://doi.org/10.5281/zenodo.20645403}{10.5281/zenodo.20645403}).
...
All numerical values in this section are exported from \texttt{results/} via \texttt{experiments.export\_manuscript\_figures} into \texttt{manuscript\_data/} (reproducible with \texttt{py -3.13 -m experiments.summarize baseline} and \texttt{py -3.13 -m experiments.sweep all}).
...
\addplot[accent, thick] table[x=step, y=loss, col sep=space]
  {manuscript_data/loss_curve_d5_seed0.dat};
...
\end{semilogyaxis}
\end{tikzpicture}
\caption{Training loss (log scale) over 500 Adam steps (seed~0, degree~5). Curve data from \texttt{manuscript\_data/loss\_curve\_d5\_seed0.dat}.}
\label{fig:loss}
\end{figure}
...
\addplot[accent, only marks, mark=*, mark size=1.8pt, opacity=0.75]
  table[x=index, y=train_mse, col sep=space]
  {manuscript_data/multiseed_d5_train_mse.dat};
\addplot[accent, dashed, thick] coordinates {(-0.4,6.3e-5) (2.4,6.3e-5)};
\node[font=\scriptsize, anchor=south] at (axis cs:1,3.5e-5) {30 seeds, $d=5$};
\node[font=\scriptsize, anchor=west, accent] at (axis cs:2.05,8e-5) {median $6.3\times10^{-5}$};
\end{semilogyaxis}
\end{tikzpicture}
\caption{Per-seed train MSE at degree~5 ($n=30$). Dashed line: median.}
\label{fig:multiseed}
\end{figure}
...
\end{semilogyaxis}
\end{tikzpicture}
\caption{Mapped train MSE for gradient vs.\ analytic baselines (degree~5, seed~0).}
\label{fig:baseline}
\end{figure}
...
\addplot[accent, thick, mark=*, mark size=2pt]
  table[x=degree, y=learned, col sep=space] {manuscript_data/scaling_table.dat};
\addlegendentry{Gradient Adam}
\addplot[black!70, thick, dashed, mark=square*, mark size=2pt]
  table[x=degree, y=analytic_mapped, col sep=space] {manuscript_data/scaling_table.dat};
\addlegendentry{Analytic (mapped)}
\end{semilogyaxis}
\end{tikzpicture}
\caption{Degree scaling (seed~0): learned vs.\ mapped analytic train MSE.}
\label{fig:scaling}
\end{figure}
...
\addplot[gray, dashed, thick]
  table[x=x, y=target, col sep=space] {manuscript_data/poly_fidelity_d5_seed0.dat};
\addlegendentry{$P_{\mathrm{target}}$ (Chebyshev)}
\addplot[accent, thick, densely dashed]
  table[x=x, y=learned, col sep=space] {manuscript_data/poly_fidelity_d5_seed0.dat};
\addlegendentry{Trained $\langle X\rangle$}
\legend{at={(0.97,0.95)}, anchor=north east, font=\scriptsize,
         draw=lightrule, fill=white}
\nextgroupplot[
  xlabel={Signal $x$},
  ylabel={Residual},
...
\addplot[accent!70, thick]
  table[x=x, y=residual, col sep=space] {manuscript_data/poly_fidelity_d5_seed0.dat};
\addplot[gray!50, thin] coordinates {(-1,0) (1,0)};
\end{groupplot}
\end{tikzpicture}
\caption{Polynomial fidelity (seed~0). \textbf{Top:} target vs.\ circuit output from exported phases. \textbf{Bottom:} pointwise residual on a dense grid.}
\label{fig:poly}
\end{figure}
...
The implementation, experiment scripts, audit trail (\texttt{docs/AUDIT\_TRAIL.md}), and figure data are available at \url{https://github.com/rosspeili/trainable-qsp-angles}~\cite{peili_trainable_qsp_2026} (DOI: \href{https://doi.org/10.5281/zenodo.20645403}{10.5281/zenodo.20645403}). Regenerate tables and plots with \texttt{py -3.13 -m experiments.sweep all}, \texttt{py -3.13 -m experiments.summarize baseline}, and \texttt{py -3.13 -m experiments.export\_manuscript\_figures}.
