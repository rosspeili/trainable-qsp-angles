...
\title{%
  ...
  {\normalsize\normalfont\color{accent}
   A Differentiable Circuit Approach Using PennyLane, JAX, and Optax}
}
...
\small
\textbf{Abstract.}\quad
Quantum Signal Processing (QSP) is a foundational algorithmic primitive that encodes polynomial transformations of a scalar signal into a single-qubit quantum circuit via a structured sequence of phase-shifted oracle calls. The canonical approach to constructing QSP circuits requires analytically computing the necessary phase angles from a target polynomial --- a well-studied but numerically sensitive problem. This paper presents an alternative paradigm: learning QSP phase angles from random initialization using gradient-based optimization, with no analytic solver required. We demonstrate this approach through a concrete implementation built on PennyLane and JAX, training a degree-5 QSP circuit to approximate a Chebyshev polynomial expansion of $\sin(x)$ via Adam gradient descent. Starting from random phase angles, the circuit converges to a final mean squared error of $4.82 \times 10^{-4}$ within 500 optimization steps. We identify and resolve a non-obvious incompatibility between PennyLane's \texttt{qml.QSVT} primitive and JAX tracing, and provide a flat-circuit implementation pattern that preserves end-to-end differentiability. The full code and an interactive Colab notebook are publicly available under the Apache~2.0 license.
...
We demonstrate this via a publicly available implementation~\cite{peili_trainable_qsp_2026} built on PennyLane~\cite{pennylane2022} and JAX~\cite{jax2018github}. Along the way, we identify and resolve a non-obvious incompatibility between PennyLane's built-in QSVT primitive and JAX's tracing mechanism, and provide the correct flat-circuit implementation pattern.

\begin{tcolorbox}[contribbox]
\begin{itemize}\setlength\itemsep{3pt}
  \item[\textbf{C1.}] We identify that PennyLane's \texttt{qml.QSVT} primitive captures operator values at circuit-build time, breaking JAX differentiability through phase angles, and provide a flat-circuit workaround that preserves end-to-end gradient flow.
  \item[\textbf{C2.}] We demonstrate gradient-based learning of QSP phase angles from random initialization, converging from MSE $\approx 1.44$ to $4.82\times10^{-4}$ in 500 Adam steps on a degree-5 Chebyshev target.
...
PennyLane's \texttt{qml.QSVT} primitive constructs operator objects at circuit-build time, capturing concrete numerical values that exit JAX's abstract trace and produce zero gradients through the phase angles. The fix is a \emph{flat circuit} of primitive gates, keeping phase angles as live JAX arrays throughout:

\begin{lstlisting}[caption={Flat-circuit QSP implementation. Expanding each oracle $W(x)=H\,R_Z\,H$ inline preserves end-to-end JAX differentiability. Note: the source uses \texttt{import pennylane as qp}; \texttt{qml} is the conventional alias used in PennyLane documentation.}, label={lst:circuit}]
...
\begin{tabularx}{\textwidth}{@{} l l X @{}}
  ...
  \midrule
  Quantum simulation  & PennyLane 0.44+   & QNode execution and gate library \\
  ...
  \bottomrule
\end{tabularx}
\caption{Technology stack. All components interoperate through JAX's functional transformation interface~\cite{peili_trainable_qsp_2026}.}
\label{tab:stack}
\end{table}
...
The flat-circuit pattern described in Section~3.3 is the correct workaround for JAX-based variational optimization of QSP circuits, and is expected to generalize to multi-qubit block-encoding extensions. JAX transforms functions by tracing them with abstract values; any component that captures concrete arrays at construction time exits the trace. This is not a PennyLane bug --- it is a consequence of JAX's design --- but it is a non-obvious pitfall for practitioners combining the two frameworks.
