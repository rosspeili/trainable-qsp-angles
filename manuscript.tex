...
Table~\ref{tab:baseline} compares gradient Adam to analytic solvers on the \emph{same flat training circuit}. Mapped train MSE applies the convention bridge $\phi_{\mathrm{flat}}=\pi/2-\phi_{\mathrm{chao}}$ (Section~\ref{sec:limitations}); raw solver angles without mapping yield much higher MSE (footnote).
...
\begin{tabular}{@{} l r r r @{}}
  ...
  \bottomrule
\end{tabular}
\caption{Baseline comparison at degree~5, seed~0. Mapped analytic rows use the shared Chebyshev$\to$pyqsp$\to$flat convention. Without mapping, raw PennyLane and Chao train MSE are $0.59$ and $0.95$ respectively; Chao native reconstruction error is $1.3\times10^{-4}$.}
\label{tab:baseline}
\end{table}
...
\begin{itemize}\setlength\itemsep{2pt}
  \item \textbf{Convention mapping.} Analytic baselines require $\phi_{\mathrm{flat}}=\pi/2-\phi_{\mathrm{chao}}$ for fair comparison on our flat circuit; mapped analytic MSE remains above gradient MSE ($\sim10^{-3}$ at $d=5$, higher at some degrees).
...
Within the scope of this benchmark, gradient-learned QSP phases are most compelling when the training objective \emph{is} the flat differentiable circuit---for example, QSP blocks inside variational pipelines with implicit targets. Immediate extensions include multi-seed scaling at higher degrees, off-grid generalization tests, tighter convention alignment for analytic baselines (sym\_qsp / machine precision where available), and optional cross-SDK validation. The combination of interpretable polynomial structure and automatic differentiation remains a practical template for quantum subroutine design when the loss is end-to-end differentiable.
