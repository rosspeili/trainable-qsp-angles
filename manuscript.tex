...
\begin{tikzpicture}[remember picture, overlay]
  \node[anchor=north west, xshift=0.15cm, yshift=-0.35cm] at (current page.north west) {%
...
All numerical values in this section are hardcoded in \texttt{manuscript\_numbers.tex} (included at compile time). Provenance: \texttt{results/paper/}, \texttt{results/t1\_degree5/}, \texttt{results/scaling/}, and \texttt{results/ablation/} JSON/CSV logs generated on 2026-06-11 by \texttt{py -3.13 -m experiments.summarize baseline} and \texttt{py -3.13 -m experiments.sweep}. Regenerate figure coordinates with \texttt{py -3.13 -m experiments.generate\_manuscript\_numbers}.
...
The QSP loss landscape is non-convex. At degree~5, 30/30 seeds converged below $10^{-3}$ train MSE with uniform initialization in $[-0.5,0.5]$. At $d=15$, hold-out error grows ($\sim2.3\times10^{-2}$) while train MSE remains below $10^{-3}$, indicating grid-limited generalization rather than optimization failure. Higher degrees may require curriculum warm-starts or second-order methods; we leave systematic study to future work.
...
\begin{itemize}\setlength\itemsep{2pt}
  ...
  \item \textbf{Scaling statistics.} Degree scaling uses one seed per degree; multi-seed stats are reported at $d=5$ only.
